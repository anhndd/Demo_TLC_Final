...
class SumoSimulation:
	...
	def plot_delay(self,data):
		...
		plt.plot(data, label = 'Agent 1')
		plt.legend(loc='upper right')
		plt.pause(0.001)  # pause a bit so that plots are updated

		# save img so sánh giữa FIXED vs MODEL (AWT_t)
		# if (self._steps == 5400):
		# 	fig = plt.gcf()
		# 	fig.set_size_inches(20, 11.25)
		# 	fig.savefig(constants.comparation_path + 'AWT_t.png', dpi=96)
...
def save_comparation():
	...
	if not constants.is_test_random:
		...
		avg_waiting_time_RANDOM = np.load(fixed_path+'RANDOM/_avg_waiting_time_per_veh.npy')

		# avg waiting_time (model)
		avg_waiting_time_RANDOM_model = np.load(model_path+'RANDOM/_avg_waiting_time_per_veh_RANDOM.npy')
		
		print('Compare AWT RANDOM: ')
		print('Fixed: ', avg_waiting_time_RANDOM)
		print('Agent: ', avg_waiting_time_RANDOM_model)
		print('Improvement: ', 1 - avg_waiting_time_RANDOM_model/avg_waiting_time_RANDOM)
		print('\n')
...
def main():
	...
	os.makedirs(os.path.dirname(plot_path + 'RANDOM/'), exist_ok=True)
	...
	for simu_type in simu_types:
		...
		if simu_type == 0:
			...
			plot_path += 'RANDOM/'
			constants.fixed_neg_reward = np.load(constants.plot_path_fixed_sys +'RANDOM/delay_array_fix.npy')
		...
		if simu_type == 'RANDOM':
			np.save(constants.plot_path_trained_model + 'RANDOM/_avg_waiting_time_per_veh_RANDOM.npy', sumo_simulation._avg_waiting_time_per_veh_RANDOM)
