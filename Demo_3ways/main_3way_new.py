...
class SumoSimulation:
    ...
    def cal_yellow_duration(self, old_action):
        v_on_road = []
        lane_ids = ["bottom_in","top_in"] if (old_action == 0) else ["right_in"]
        for id in lane_ids:
    ...
    def _set_green_phase(self, phase_number):
        if phase_number == 0:
            traci.trafficlight.setPhase(constants.light_id, 0)          # bot-tom
        elif phase_number == 1:
...
def main():
    ...
    while current_episode < total_episodes:
        ...
        sumo_simulation.run_one_episode_training(current_episode, total_episodes)            # run for training with RANDOM_ROUTE
        # sumo_simulation.run_one_episode_on_STL(current_episode, total_episodes)              # run for LOGGING with FIXED_ROUTE
